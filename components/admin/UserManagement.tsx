...
const roles = Object.keys(ROLE_DEFINITIONS) as CommunityRole[];

export function UserManagement() {
  const [query, setQuery] = useState("");
  const [users, setUsers] = useState<ManagedUser[]>([]);
  const [selected, setSelected] = useState<ManagedUser | null>(null);
  const [selectedRoles, setSelectedRoles] = useState<CommunityRole[]>([]);
  ...
  function toggleRole(role: CommunityRole) {
  ...
  return <section className="admin-stack" dir="rtl">
    <div className="admin-block">
      ...
      <div className="admin-user-suggestions">{users.slice(0, 12).map((user) => <button type="button" key={user.id} onClick={() => chooseUser(user)} className={`admin-user-suggestion${selected?.id === user.id ? " is-active" : ""}`}><span className="relative h-9 w-9 overflow-hidden rounded-full bg-white/10">{user.avatarUrl ? <img src={user.avatarUrl} alt="" className="h-full w-full object-cover" /> : <UserRound size={17} className="m-2" />}<i className={`absolute bottom-0 end-0 h-2.5 w-2.5 rounded-full border-2 border-[#0d121f] ${user.online ? "bg-emerald-400" : "bg-white/20"}`} /></span><span className="min-w-0 text-start"><strong className="block truncate">{user.name}</strong><small className="block truncate text-white/40">{user.email}</small></span><b className="ms-auto text-xs text-[#00F0FF]">{user.xp.toLocaleString("en")} XP</b></button>)}</div>
    </div>
    {selected && <div className="admin-block"><div className="admin-block__heading"><h4>{selected.name}</h4><span className="text-xs text-white/50">{selected.email} · {selected.online ? "متصل" : "غير متصل"}</span></div><div className="admin-grid">{roles.map((role) => <label key={role} className="admin-field flex-row items-center gap-3"><input type="checkbox" checked={selectedRoles.includes(role)} onChange={() => toggleRole(role)} disabled={role === "member" && selectedRoles.length > 1} /><span><strong style={{ color: ROLE_DEFINITIONS[role].color }}>{ROLE_DEFINITIONS[role].label}</strong><small className="block text-white/40">{ROLE_DEFINITIONS[role].permissions.join(" · ") || "صلاحيات اللاعب الأساسية"}</small></span></label>)}</div><button type="button" className="gaming-button" onClick={save} disabled={saving}><Save size={15} /> {saving ? "جارٍ الحفظ..." : "حفظ الرتب"}</button>{notice && <p className="admin-hint">{notice}</p>}</div>}
  </section>;
}