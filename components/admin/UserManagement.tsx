...
export function UserManagement() {
  ...
  return <section className="admin-stack" dir="rtl">
    <div className="admin-block">
      ...
      <div className="admin-user-suggestions">{users.slice(0, 12).map((user) => <button type="button" key={user.id} onClick={() => chooseUser(user)} className={`admin-user-suggestion${selected?.id === user.id ? " is-active" : ""}`}><span className="admin-user-suggestion__avatar relative shrink-0 overflow-hidden rounded-full bg-white/10">{user.avatarUrl ? <img src={user.avatarUrl} alt="" /> : <UserRound size={17} className="m-2" />}<i className={`absolute bottom-0 end-0 h-2.5 w-2.5 rounded-full border-2 border-[#0d121f] ${user.online ? "bg-emerald-400" : "bg-white/20"}`} /></span><span className="min-w-0 text-start"><strong className="block truncate">{user.name}</strong><small className="block truncate text-white/40">{user.email}</small></span><b className="ms-auto text-xs text-[#00F0FF]">{user.xp.toLocaleString("en")} XP</b></button>)}</div>
    </div>
