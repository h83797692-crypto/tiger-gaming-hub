...
import { Clapperboard, Heart, LogIn, Upload } from "lucide-react";
...
export function ClipHall() {
  ...
  async function vote(clipId: string) {
    const response = await fetch("/api/clips", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ clipId }) });
    const payload = await response.json();
    if (!response.ok) { setNotice(payload.error ?? "تعذر تسجيل التصويت"); return; }
    setClips((current) => current.map((clip) => clip.id === clipId ? { ...clip, votes: payload.votes } : clip));
  }
